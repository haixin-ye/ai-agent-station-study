...
DROP TABLE IF EXISTS `agent_recall_eval_hit`;
DROP TABLE IF EXISTS `agent_recall_eval_case_result`;
DROP TABLE IF EXISTS `agent_recall_eval_run`;
DROP TABLE IF EXISTS `agent_recall_eval_case`;
DROP TABLE IF EXISTS `agent_recall_eval_corpus_item`;
DROP TABLE IF EXISTS `agent_recall_eval_dataset`;
...
DROP TABLE IF EXISTS `agent_rag_code_file`;
DROP TABLE IF EXISTS `agent_rag_chunk`;
DROP TABLE IF EXISTS `agent_rag_document`;
...
CREATE TABLE `agent_rag_code_file` (
  `id` bigint unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE `agent_recall_eval_dataset` (
  `id` bigint unsigned NOT NULL AUTO_INCREMENT,
  `dataset_id` varchar(64) NOT NULL,
  `name` varchar(160) NOT NULL,
  `description` varchar(1024) DEFAULT NULL,
  `status` varchar(32) NOT NULL,
  `eval_user_id` varchar(96) NOT NULL,
  `eval_session_id` varchar(96) NOT NULL,
  `corpus_count` int NOT NULL DEFAULT 0,
  `ready_corpus_count` int NOT NULL DEFAULT 0,
  ...
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_recall_eval_dataset_id` (`dataset_id`),
...
CREATE TABLE `agent_recall_eval_corpus_item` (
  `id` bigint unsigned NOT NULL AUTO_INCREMENT,
  `corpus_item_id` varchar(64) NOT NULL,
  `dataset_id` varchar(64) NOT NULL,
  ...
  `parent_source_id` varchar(128) DEFAULT NULL,
  ...
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_recall_eval_corpus_item_id` (`corpus_item_id`),
...
CREATE TABLE `agent_recall_eval_case` (
  `id` bigint unsigned NOT NULL AUTO_INCREMENT,
  ...
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_recall_eval_case_id` (`case_id`),
...
CREATE TABLE `agent_recall_eval_run` (
  `id` bigint unsigned NOT NULL AUTO_INCREMENT,
  ...
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_recall_eval_run_id` (`evaluation_run_id`),
  KEY `idx_recall_eval_run_dataset` (`dataset_id`, `created_at`),
  KEY `idx_recall_eval_run_status` (`status`, `updated_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Recall evaluation run';

CREATE TABLE `agent_recall_eval_case_result` (
  `id` bigint unsigned NOT NULL AUTO_INCREMENT,
  `case_result_id` varchar(64) NOT NULL,
  `evaluation_run_id` varchar(64) NOT NULL,
  `case_id` varchar(64) NOT NULL,
  `status` varchar(32) NOT NULL,
  `retrieval_latency_ms` bigint DEFAULT NULL,
  `planner_latency_ms` bigint DEFAULT NULL,
  `hit` tinyint(1) DEFAULT NULL,
  `precision_at_k` decimal(14,8) DEFAULT NULL,
  `recall_at_k` decimal(14,8) DEFAULT NULL,
  `reciprocal_rank` decimal(14,8) DEFAULT NULL,
  `ndcg_at_k` decimal(14,8) DEFAULT NULL,
  `average_precision_at_k` decimal(14,8) DEFAULT NULL,
  `planner_status` varchar(32) DEFAULT NULL,
  `planner_reason` varchar(2048) DEFAULT NULL,
  `planner_selected_ids_json` longtext,
  `planner_output_json` longtext,
  `failure_stage` varchar(64) DEFAULT NULL,
  `failure_code` varchar(128) DEFAULT NULL,
  `failure_message` varchar(2048) DEFAULT NULL,
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_recall_eval_case_result_id` (`case_result_id`),
...
CREATE TABLE `agent_recall_eval_hit` (
  `id` bigint unsigned NOT NULL AUTO_INCREMENT,
  ...
  `collection_type` varchar(64) DEFAULT NULL,
  `source_type` varchar(64) DEFAULT NULL,
  `source_id` varchar(128) NOT NULL,
  `parent_source_id` varchar(128) DEFAULT NULL,
  `score` decimal(14,8) DEFAULT NULL,
  `expected_grade` int DEFAULT NULL,
  `selected_by_planner` tinyint(1) DEFAULT NULL,
  `candidate_json` longtext,
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_recall_eval_hit_id` (`hit_id`),
