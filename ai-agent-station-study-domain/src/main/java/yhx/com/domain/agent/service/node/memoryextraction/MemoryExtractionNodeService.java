package yhx.com.domain.agent.service.node.memoryextraction;
...
public class MemoryExtractionNodeService {

    private final NodeInvocationPipeline nodeInvocationPipeline;
    private final NodeInvocationProfileVO defaultProfile;

    public MemoryExtractionNodeService(NodeInvocationPipeline nodeInvocationPipeline) {
        this(nodeInvocationPipeline, null);
    }

    public MemoryExtractionNodeService(NodeInvocationPipeline nodeInvocationPipeline, NodeInvocationProfileVO defaultProfile) {
        this.nodeInvocationPipeline = nodeInvocationPipeline;
        this.defaultProfile = defaultProfile;
    }

    public MemoryExtractionOutputVO extract(MemoryExtractionInputVO input, String agentId, NodeInvocationProfileVO profile) {
        ...
        NodeInvocationResult result = nodeInvocationPipeline.invoke(NodeInvocationCommand.builder()
                ...
                .agentId(agentId)
                .componentCode(AgentComponentCodeEnumVO.MEMORY_EXTRACTOR.name())
                .contractVersion(firstNonBlank(effectiveProfile == null ? null : effectiveProfile.getContractVersion(), "memory-extraction-output-v1"))
                ...
                .inputView(input)
                .build());
        if (result.getTypedOutput() instanceof MemoryExtractionOutputVO output) {
            return output;
        }
        return null;
    }
