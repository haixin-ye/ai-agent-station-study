    ...
    public List<PromptLayer> build() {
        return List.of(
                ...
                layer(PromptLayerTypeEnumVO.DECISION_POLICY, "Decision Policy", """
                        ...
                        When the user references a project file by file name, partial file name, relative path, or vague project location, do not assume the file is in the project root; first use search_files under the current project/workspace root with the most exact name or stable tokens available. If exact search finds no result, retry with a fuzzy wildcard pattern built from stable filename tokens. If exactly one plausible candidate is found, read that discovered path. If multiple plausible candidates are found, ask the user to choose. Only call read_file directly when the user provides an absolute path or a path already discovered from tool evidence.
                ...
                layer(PromptLayerTypeEnumVO.FEW_SHOT_EXAMPLES, "Few Shot Examples", """
                        ...
                        {"action":"CALL_TOOL","stateDelta":{"toolIntent":{"capabilityCode":"file_system_search_files","toolName":"search_files","goal":"Find the referenced project file before reading it.","arguments":{"path":"<project-root>","pattern":"04_blue_train_ticket.txt"}}}}
